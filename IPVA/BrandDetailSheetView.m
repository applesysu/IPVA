...
@implementation BrandDetailSheetView
...
- (id)initWithFrame:(CGRect)frame andTitles:(NSArray *)titles andPropertyname:(NSArray *)propertyName andData:(NSArray *)mydata
{
    ...
    if (self) {
        ...
        //初始化表头行的各个数据名目
        titleRowView = [[UIView alloc] initWithFrame:CGRectMake(10, 10, [self.titleArray count] * 130, 50)];
        titleRowView.backgroundColor = [UIColor grayColor];
        for(int i=0;i<[titleArray count];i++)
        {
            UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(10+130*i, 10, 110, 30)];
        ...
        //每个记录的table
        rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(10, 60, [self.titleArray count] * 130, 500)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BrandAnalysisDetailData *tempData = (BrandAnalysisDetailData *)[self.data objectAtIndex:indexPath.row];
    label.text = [NSString stringWithFormat:@"%ld", tempData.rankNumber];
    
    label = (UILabel *)[cell.contentView viewWithTag:101];
    tempData = (BrandAnalysisDetailData *)[self.data objectAtIndex:indexPath.row];
    label.text = tempData.squareName;
    
    label = (UILabel *)[cell.contentView viewWithTag:102];
    tempData = (BrandAnalysisDetailData *)[self.data objectAtIndex:indexPath.row];
    label.text = [NSString stringWithFormat:@"%ld", tempData.storeID];
