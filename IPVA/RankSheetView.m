...
@implementation RankSheetView

@synthesize leftTableView, leftScrollView, rightTableView, rightScrollView, titleArray, titleRowView, nameLabels,datas;

- (id)initWithFrame:(CGRect)frame andTitles:(NSArray *)titles andNamelabels:(NSArray *)namelabelarray andDatas:(NSArray *)mydata
{
    ...
    if (self) {
        ...
        leftScrollView.contentSize = CGSizeMake(self.frame.size.width, 200);
        rightScrollView.contentSize = CGSizeMake([self.titleArray count] * 80, 200);
        rightScrollView.directionalLockEnabled = YES;
        ...
        //每个记录的table
        rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, [self.titleArray count] * 80, [self.nameLabels count]*50)];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
	...
	rightTableView.frame = CGRectMake(0, 0, [self.titleArray count] * 80,1040);
	[rightScrollView addSubview:rightTableView];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	if(tableView == leftTableView)
	{
        ...
        for (int i = 0; i < [self.titleArray count]; i++)
        {
            label = (UILabel *)[cell.contentView viewWithTag:(100+i)];
//            label.text  = [NSString stringWithFormat:@"%f", [self.datas objectAtIndex:i]];
        }
